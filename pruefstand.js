...
if(dateiArg) warn('--datei gesetzt: Abschnitt 0 entfaellt, geprueft wird ' + HTML);
  ...
  /* Der Bau selbst rechnet den Soll-Stand (bauen.js als Modul): dieselbe Verkettung,
     derselbe Stand-Einsatz. Der Stand kommt aus der ausgelieferten Datei zurueck -
     so bleibt der Vergleich ein Byte-Beweis, obwohl das Datum darin steht. */
  const bau = require('./bauen.js');
  const erstes = path.join(ORDNER, 'werkstatt-pipeline.html');
  const standIst = fs.existsSync(erstes) ? bau.standLesen(fs.readFileSync(erstes, 'utf8')) : null;
  let gebaut = null;
  try{ gebaut = bau.zusammensetzen(standIst); }
  catch(e){ console.log('  X Bau nicht moeglich: ' + e.message); process.exit(2); }
  const man = gebaut.man, soll = gebaut.buf;
  if(standIst) ok('Stand der ausgelieferten Datei: ' + standIst); else { console.log('  X Die ausgelieferte Datei traegt keinen Stand - "node bauen.js" ausfuehren.'); process.exit(2); }
  {
    const swZiel = path.join(ORDNER, 'docs', 'sw.js'), stZiel = path.join(ORDNER, 'docs', 'stand.txt');
    if(fs.existsSync(swZiel) && fs.readFileSync(swZiel, 'utf8') === bau.swText(standIst)) ok('docs/sw.js gehoert zu diesem Stand (Cache-Name, waitUntil, Netz zuerst)');
    else { console.log('  X docs/sw.js fehlt oder gehoert zu einem anderen Stand'); process.exit(2); }
    if(fs.existsSync(stZiel) && fs.readFileSync(stZiel, 'utf8') === standIst + '\n') ok('docs/stand.txt traegt denselben Stand');
    else { console.log('  X docs/stand.txt fehlt oder weicht ab'); process.exit(2); }
  }
  let schief = 0;
...
/* Der LETZTE Skriptblock ist die App. Seit dem 19.09.2026 steht im Kopf ein eigenes
   <script> mit dem Stand - ein nicht-gieriges Muster ab dem ersten <script> schnitt
   von dort bis zum Schluss und nahm die halbe Seite als "Code" mit. */
const m = (() => {
  const ende = quelltext.lastIndexOf('</script>');
  const anfang = ende < 0 ? -1 : quelltext.lastIndexOf('<script>', ende);
  return anfang < 0 ? null : [null, quelltext.slice(anfang + '<script>'.length, ende)];
})();
...
/* --- 37. Design-Uebertragung und CAM-Anbindung (19.09.2026) ------------
   Freigabe der Nachtdurchsicht am 19.09.2026 (MORGENBERICHT-DESIGN.md der Dreh-App,
   Teil 3): Kopfzeile wie die Schwestern, Dunkelmodus ueber die Token, Kachel-
   felder, Leerzustand mit drei Wegen, Anleitung in der App, Stand und Service-
   Worker (Abschnitt 0) - und die Funktionsluecken: Zeiten aus dem CAM (mit
   Ruestzeit aus der Werkzeugzahl und Bearbeitungsseiten), Programm je Arbeits-
   gang auf Laufkarte und Zettel, Werkstoffe als Datei fuer die Dreh-App,
   Drehkontur als Programm der Dreh-App, Kernloch als Gewinde.
   GEGENPROBEN in gegenproben.js: Gewinde blind, Ruestzeit ohne Werkzeuge,
   start-Element weg, Ruestzeit nicht ueberschreibbar.                      */
console.log('\n37) Design-Uebertragung und CAM-Anbindung');
{
  const camZeitPruefen = hole('camZeitPruefen'), camZeitUebernahme = hole('camZeitUebernahme');
  const wpWerkstoffeDatei = hole('wpWerkstoffeDatei'), fGewindeAusKernloch = hole('fGewindeAusKernloch');
  const fDrehProfil = hole('fDrehProfil'), dpAlsDrehProgramm = hole('dpAlsDrehProgramm'), fStepModell = hole('fStepModell');
  const gangNeu = hole('gangNeu'), auftragGaenge = hole('auftragGaenge'), neuerAuftrag = hole('neuerAuftrag');
  const planMinuten = hole('planMinuten');
  /* (1) Kopfzeile, Dunkelmodus, Kacheln, Anleitung - der Bau */
  gleich('Kopf: ein Emblem', (quelltext.match(/<span class="emblem"/g) || []).length, 1);
  gleich('Kopf: sechs Reiter mit Glyphe', (quelltext.match(/<button id="tab[A-Za-z]+"[^>]*>\s*<svg/g) || []).length, 6);
  ['kopfChip', 'kopfStand', 'dunkelBtn', 'baBtn', 'baPanel', 'baToc', 'camKarte', 'btnCamLaden', 'btnWstExport', 'btnDrehkontur'].forEach(id => {
  ...
  gleich('Anleitung: Spruenge aus den Feldern (label[data-ba])', (quelltext.match(/<label for="[^"]+" data-ba="ba-\d\d">/g) || []).length >= 10, true);
  gleich('Uebersicht wird beim Start gemalt', /if\(typeof wUebersichtMalen === 'function'\) wUebersichtMalen\(\);/.test(quelltext), true);
  gleich('Leerzustand: drei Wege', ['ueLeerDemo', 'ueLeerStep', 'ueLeerAuftrag'].every(id => quelltext.indexOf('id="' + id + '"') > 0), true);
  gleich('Meldungen entdoppelt', /letzte\.className === d\.className && letzte\.innerHTML === d\.innerHTML\) return;/.test(quelltext), true);
  gleich('Stand: Pruefung gegen stand.txt ohne stillen Neustart', /fetch\('stand\.txt', \{cache:'no-store'\}\)/.test(quelltext) && !/location\.reload\(\)/.test(quelltext), true);
  /* (2) Zeiten aus dem CAM - DOM-frei */
  const J = {format:'dz-cam-zeit', version:1, quelle:'drehen', datum:'2026-09-19T08:00:00Z', programm:{name:'O00009010', maschine:'Monforts 1500'},
             zeiten:{komplett_min:26.233, schnitt_min:19.733, werkzeuge:6, wechsel:13}, aufspannungen:2};
  gleich('Zeit-Datei: gueltig', camZeitPruefen(J).length, 0);
  gleich('Zeit-Datei: falsches Format faellt auf', camZeitPruefen(Object.assign({}, J, {format:'angebot'})).length > 0, true);
  gleich('Zeit-Datei: ohne komplett_min faellt auf', camZeitPruefen(Object.assign({}, J, {zeiten:{werkzeuge:6}})).length > 0, true);
  const U = camZeitUebernahme(J, KALK_VORGABEN);
  nahe('Hauptzeit = Laufzeit mit Werkzeugwechseln', U.ueber.hauptzeit, 26.233, 1e-9);
  ...
  {
    const d = neuerDatensatz(); d.teil.klasse = 'drehteil_einfach'; d.teil.volumen_cm3 = 100; d.rohteil.volumen_cm3 = 160; d.teil.werkstoff = 'S235';
    const ein = {teil:d.teil, rohteil:d.rohteil, vorgaben:KALK_VORGABEN, werkstoff:'S235', toleranz:'mittel', oberflaeche:'normal', seiten:1, stueck:10, versandArt:'versand', ueber:{}};
    const r0 = kalkRechnen(ein), r1 = kalkRechnen(Object.assign({}, ein, {ueber:{ruestzeit:28}}));
    gleich('Ruestzeit ohne Ueberschreibung = Klassenwert', r0.zeiten.ruestzeit, KALK_VORGABEN.ruesten.drehteil_einfach);
    gleich('ueberschriebene Ruestzeit steht in den Zeiten', r1.zeiten.ruestzeit, 28);
    nahe('  und in der Ruestumlage (28 min / 60 x Satz / 10 Stueck)', r1.preise.ruesten, 28 / 60 * r1.ruestSatz / 10, 1e-9);
    gleich('  und ist als ueberschrieben markiert', r1.ueberschrieben.ruestzeit, true);
    gleich('Ruestzeit-Zeile im Blatt ist ein Eingabefeld', /z\('ruestzeit', 'R&uuml;stzeit je Auftrag'/.test(quelltext), true);
  }
  /* (4) Kernloch -> Gewinde */
  gleich('Kernloch 8,5 ist M10', fGewindeAusKernloch(8.5), 'M10');
  gleich('Kernloch 6,8 ist M8', fGewindeAusKernloch(6.8), 'M8');
  gleich('8,4 ist ein Loch', fGewindeAusKernloch(8.4), null);
  {
    const d = neuerDatensatz(); d.teil.volumen_cm3 = 100; d.rohteil.volumen_cm3 = 160;
    const ein = {teil:d.teil, rohteil:d.rohteil, vorgaben:KALK_VORGABEN, werkstoff:'S235', toleranz:'mittel', oberflaeche:'normal', seiten:1, stueck:1, versandArt:'versand', ueber:{}};
    d.teil.bohrungen = [{d:8.5, tiefe:12, durch:true}];
    const ohne = kalkRechnen(ein).zeiten.nebenzeit;
    d.teil.bohrungen = [{d:8.5, tiefe:12, durch:true, gewinde:'M10'}];
    const mit = kalkRechnen(ein);
    nahe('ein Gewinde kostet nebenzeit_je_gewinde zusaetzlich', mit.zeiten.nebenzeit - ohne, KALK_VORGABEN.zeiten.nebenzeit_je_gewinde, 1e-9);
    gleich('  und wird gezaehlt', mit.zeiten.gewinde, 1);
    /* Am Pruefkoerper: ein Quader mit 8,5er Bohrung ergibt ein Kernloch M10 */
    const g = fGeometrie(PK.quader(40, 30, 10, 8.5), 'kern.step', KALK_VORGABEN);   /* vierter Wert = Bohrungsdurchmesser */
    gleich('Geometrie: die 8,5er Bohrung des Quaders traegt gewinde M10', (g.teil.bohrungen[0] || {}).gewinde, 'M10');
  }
  /* (5) Werkstoffe als Datei */
  {
    const w = wpWerkstoffeDatei(KALK_VORGABEN);
    gleich('Werkstoff-Datei: Format und Version', w.format + '/' + w.version, 'wp-werkstoffe/1');
    gleich('  alle Werkstoffe mit Name, Gruppe, Dichte, Preis, Faktor', w.werkstoffe.length === KALK_VORGABEN.werkstoffe.length && w.werkstoffe.every(x => x.name && x.gruppe && x.dichte > 0 && typeof x.preis === 'number' && x.faktor > 0), true);
  }
  /* (6) Drehkontur als Programm der Dreh-App, am Pruefkoerper (Stufenwelle mit Bohrung) */
  {
    const M = fStepModell(PK.drehteil([[40, 30], [60, 20]], 16));
    const p = fDrehProfil(M);
    gleich('Drehteil-Pruefkoerper: Profil gelesen (Rohr)', p.ok && !p.voll, true);
    const d = neuerDatensatz(); d.teil.name = 'Huelse'; d.teil.zeichnungsnr = 'Z-4711'; d.rohteil = {form:'rohr', masse:{d:63, di:16, l:52}, volumen_cm3:0};
    const prog = dpAlsDrehProgramm(p, d, KALK_VORGABEN);
    gleich('Programm: Format der Dreh-App, Version 2, radial', prog && prog.format + '/' + prog.version + '/' + prog.wertung, 'euroturn-cam-programm/2/radial');
    gleich('Programm: zwei Konturen (Aussen, Innen)', prog && prog.contours.map(c => c.name).join(','), 'Aussen,Innen');
    gleich('jede Kontur beginnt mit einem start-Element', prog && prog.contours.every(c => c.els[0].kind === 'start' && c.els.slice(1).every(e => e.kind === 'gerade')), true);
    gleich('Aussenkontur: Start an der Bohrungskante, dann Stirn auf 60, Absatz bei -20 auf 40, Ende bei -50', prog && prog.contours[0].els.map(e => e.z + '/' + e.x).join(' '), '0/16 0/60 -20/60 -20/40 -50/40');
    gleich('Innenkontur: die Bohrungswand 16 ueber 50 mm', prog && prog.contours[1].els.map(e => e.z + '/' + e.x).join(' '), '0/16 -50/16');
    gleich('Rohteil: Rohr 63/16, Laenge 52 als ZA 1 / ZI -51', prog && [prog.settings.pkForm, prog.settings.stockDia, prog.settings.pkXI, prog.settings.pkZA, prog.settings.pkZI].join(' '), 'rohr 63 16 1 -51');
    gleich('Programmnummer aus der Zeichnungsnummer', prog && prog.name, 'O00004711');
    const voll = dpAlsDrehProgramm(fDrehProfil(fStepModell(PK.drehteil([[40, 30], [60, 20]], 0))), neuerDatensatz(), KALK_VORGABEN);
    gleich('volles Teil: eine Kontur, Start auf der Achse, Zylinder', voll && voll.contours.length === 1 && voll.contours[0].els[0].x === 0 && voll.settings.pkForm === 'zylinder', true);
  }
  /* (7) Programm je Arbeitsgang bis auf den Zettel */
  {
    gleich('gangNeu traegt programm', gangNeu(1).programm, '');
    const a = neuerAuftrag(); a.stueck = 4; a.zeiten = {ruestzeit:20, stueckzeit:5}; a.maschine = 'dr1'; a.programm = 'O00009010';
    gleich('Einzelgang erbt das Programm des Auftrags', auftragGaenge(a)[0].programm, 'O00009010');
    gleich('planMinuten reicht es an die Planung durch', planMinuten(a).gaenge[0].programm, 'O00009010');
    gleich('Bloecke und Zettel tragen das Programm', /gang:g\.nr, gangName:g\.name, programm:g\.programm \|\| ''/.test(quelltext) && /programm:k\.programm \|\| ''/.test(quelltext), true);
    gleich('Laufkarte und Zettel drucken die Spalte Programm', (quelltext.match(/<th>Programm<\/th>/g) || []).length >= 3, true);
