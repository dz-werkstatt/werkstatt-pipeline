...
const faelle = [
  ...
  },
  {
    ...
    /* seit 19.09.2026 rechnet die Umlage mit dem UEBERSCHREIBBAREN Posten ruestZeit */
    suche:"  const ruesten = nimm('ruesten', ruestZeit.w / 60 * ruestSatz / stueck);",
    ersatz:"  const ruesten = nimm('ruesten', ruestZeit.w / 60 * ruestSatz);",
  ...
  /* ---- 19.09.2026: CAM-Anbindung und Design-Uebertragung (Abschnitt 37) ---- */
  {
    name:'Kernloch wird nicht mehr als Gewinde erkannt',
    suche:"  for(const [g, k] of WP_KERNLOCH) if(Math.abs(x - k) < 0.051) return g;",
    ersatz:"  for(const [g, k] of WP_KERNLOCH) if(Math.abs(x - k) < 0.0000001 && false) return g;",
    erwartet:['Kernloch 8,5 ist M10', 'Geometrie: die 8,5er Bohrung des Quaders traegt gewinde M10']
  },
  {
    name:'CAM-Zeit ohne Ruestzeit aus der Werkzeugzahl',
    suche:"    aus.ueber.ruestzeit = fRund(grund + z.werkzeuge * je, 2);",
    ersatz:"    aus.ueber.ruestzeit = fRund(grund, 2);",
    erwartet:['Ruestzeit aus sechs Werkzeugen']
  },
  {
    name:'Drehkontur-Programm ohne start-Element',
    suche:"  const start = (e) => ({ kind:'start', z:e.z, x:e.x, trans:{type:'none', val:0} });",
    ersatz:"  const start = (e) => ({ kind:'gerade', z:e.z, x:e.x, trans:{type:'none', val:0} });",
    erwartet:['jede Kontur beginnt mit einem start-Element']
  },
  {
    name:'Ruestzeit-Posten nicht mehr ueberschreibbar',
    suche:"  const ruestZeit = nimm('ruestzeit', ruestMin);",
    ersatz:"  const ruestZeit = {w:ruestMin, ueberschrieben:false};",
    erwartet:['ueberschriebene Ruestzeit steht in den Zeiten']
  }
];
