...
function kalkMalen(){
  ...
  h += z('hauptzeit', 'Hauptzeit Zerspanung' + camMarke('hauptzeit'), r.zeiten.hauptzeit, 'min');
  ...
  h += z('ruestzeit', 'R&uuml;stzeit je Auftrag' + camMarke('ruestzeit'), r.zeiten.ruestzeit, 'min');
...
/* ---- Zeiten aus dem CAM (19.09.2026, Luecke 1 des Berichts) -------------------------
   Die Zeit-Datei der Dreh- oder Fraes-App wird zu UEBERSCHRIEBENEN Posten mit Herkunft:
   sichtbar als Marke an der Zeile und als Satz unter dem Knopf. Verwerfen nimmt genau
   diese Posten zurueck, nichts sonst. */
function camMarke(k){
  return (S.cam && S.cam.felder && S.cam.felder.indexOf(k) >= 0 && S.ueber[k] != null) ? ' <span class="marke gemessen">aus DZ CAM</span>' : '';
}
function camStandMalen(){
  const e = el('camStand'), w = el('btnCamWeg'); if(!e) return;
  if(!S.cam){ e.innerHTML = 'Noch keine CAM-Zeit geladen &mdash; die Hauptzeit kommt aus dem Spanvolumen-Modell.'; if(w) w.hidden = true; return; }
  const esc = (x) => String(x == null ? '' : x).replace(/&/g, '&amp;').replace(/</g, '&lt;');
  const was = (S.cam.felder || []).map(f => f === 'hauptzeit' ? 'Hauptzeit' : f === 'ruestzeit' ? 'R&uuml;stzeit aus ' + S.cam.werkzeuge + ' Werkzeugen' : f === 'seiten' ? 'Bearbeitungsseiten ' + S.cam.seiten : f);
  e.innerHTML = '<b>Herkunft:</b> ' + esc(S.cam.herkunft) + ' &mdash; &uuml;bernommen: ' + was.join(', ') +
    '. Die Nebenzeit rechnet weiter nach Formel (Spannen und Messen stehen in keinem NC-Programm).';
  if(w) w.hidden = false;
}
function camUebernehmen(j){
  const f = camZeitPruefen(j);
  if(f.length){ meldungListe('Das ist keine Zeit-Datei von DZ CAM:', f, 'fehler'); return false; }
  if(!S.d){ meldung('Erst ein Teil laden &mdash; die CAM-Zeit geh&ouml;rt zu einem Teil.', 'warn'); return false; }
  const u = camZeitUebernahme(j, S.V);
  Object.keys(u.ueber).forEach(k => { S.ueber[k] = u.ueber[k]; });
  if(u.seiten && el('kSeiten')) el('kSeiten').value = u.seiten;
  S.cam = { format:j.format, version:j.version, quelle:j.quelle, programm:fKopie(j.programm || {}), zeiten:fKopie(j.zeiten || {}),
    aufspannungen:j.aufspannungen != null ? j.aufspannungen : null, datum:j.datum || '',
    herkunft:u.herkunft, felder:Object.keys(u.ueber).concat(u.seiten ? ['seiten'] : []), werkzeuge:(j.zeiten && j.zeiten.werkzeuge) || 0, seiten:u.seiten };
  kalkMalen(); camStandMalen();
  meldung('CAM-Zeit &uuml;bernommen: ' + u.herkunft, 'info');
  return true;
}
function camVerwerfen(){
  if(!S.cam) return;
  (S.cam.felder || []).forEach(k => { if(k !== 'seiten') delete S.ueber[k]; });
  S.cam = null; kalkMalen(); camStandMalen();
}
...
function einMalen(){
  ...
  htm('einRest',
    ...
    einFeld('zeiten.nebenzeit_je_gewinde', 'Nebenzeit je Gewinde (Kernloch erkannt)', 'min') +
    '<h3>R&uuml;stzeit aus der Werkzeugzahl (CAM-Zeitdatei)</h3>' +
    einFeld('zeiten.ruest_grund', 'Grundwert je Auftrag', 'min') +
    einFeld('zeiten.ruest_je_werkzeug', 'je Werkzeug', 'min') +
...
function angebotDatensatz(){
  ...
  d.kalkulation = {
    ...
    zeiten: Object.assign({}, r.zeiten, S.cam ? {cam:fKopie(S.cam)} : {}),
    preise: r.preise
  };
...
function angebotVerdrahten(){
    ...
    r.onload = () => {
      try{
        ...
        if(k){
          ...
          S.cam = (d.kalkulation && d.kalkulation.zeiten && d.kalkulation.zeiten.cam) ? fKopie(d.kalkulation.zeiten.cam) : null;
  ...
  on('btnCamLaden', 'click', () => el('camDatei') && el('camDatei').click());
  on('camDatei', 'change', () => {
    const f = el('camDatei').files[0]; if(!f) return;
    const r = new FileReader();
    r.onload = () => { try{ camUebernehmen(JSON.parse(String(r.result))); }catch(e){ meldung('Die Datei liess sich nicht lesen: ' + e.message, 'fehler'); } el('camDatei').value = ''; };
    r.readAsText(f);
  });
  on('btnCamWeg', 'click', camVerwerfen);
  on('btnWstExport', 'click', () => dateiSichern('werkstoffe-dz-cam.json', JSON.stringify(wpWerkstoffeDatei(S.V), null, 1)));
  on('btnWstNeu', 'click', () => {
