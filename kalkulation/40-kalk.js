...
const KALK_POSTEN = [
  ...
  'ruestzeit'   /* seit 19.09.2026 ueberschreibbar - die CAM-Zeitdatei setzt sie aus der Werkzeugzahl */
];
...
function kalkRechnen(ein){
  ...
  /* Kernloecher, die als Gewinde erkannt sind (fGewindeAusKernloch): das Gewinde ist ein
     eigener Zeitposten - Kernloch bohren zaehlt als Bohrung, das Schneiden kommt obendrauf. */
  const gew = Array.isArray(t.bohrungen) ? t.bohrungen.filter(b => b && b.gewinde).length : 0;
  ...
  const neben = nimm('nebenzeit',
    ...
    gew * (Z.nebenzeit_je_gewinde != null ? Z.nebenzeit_je_gewinde : 0.8) +
  ...
  const ruestZeit = nimm('ruestzeit', ruestMin);   /* ueberschreibbar wie jeder Posten */
  ...
  const ruesten = nimm('ruesten', ruestZeit.w / 60 * ruestSatz / stueck);
  ...
  return {
    ...
    zeiten: {
      ...
      entgraten:entgraten.w, stueckzeit:stueckzeit.w, ruestzeit:ruestZeit.w, stueck:stueck, gewinde:gew
    },
    preise: {
    ...
    ueberschrieben: {
      ...
      ruestzeit:ruestZeit.ueberschrieben
    },
    hinweise: hinweise
  };
}
