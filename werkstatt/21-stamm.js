...
function neuerAuftrag(){
  return {
    ...
    programm: '',               /* NC-Programm des Einzelgangs (O-Nummer / Klartext-Name), 19.09.2026 */
...
function gangNeu(nr){
  return { nr:nr || 1, name:'', maschine:'', ruestzeit:0, stueckzeit:0, programm:'' };
}
...
function auftragGaenge(a){
  if(a && Array.isArray(a.gaenge) && a.gaenge.length){
    return a.gaenge.map((g, i) => ({
      ...
      programm: String((g && g.programm) || ''),   /* auf Laufkarte und Zettel */
      ruestzeit: +(g && g.ruestzeit) || 0,
      stueckzeit: +(g && g.stueckzeit) || 0
    }));
  }
  const z = (a && a.zeiten) || {};
  return [{ nr:1, name:'', maschine:String((a && a.maschine) || ''), programm:String((a && a.programm) || ''),
...
function auftragAusKalkulation(d, k, zusatz){
  ...
  a.programm = String(z.programm || '');   /* aus der CAM-Zeitdatei, wenn eine geladen war */
