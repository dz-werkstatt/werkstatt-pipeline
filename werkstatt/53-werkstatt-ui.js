...
function wGaengeMalen(a){
  ...
  let h = '<tr><th>Nr.</th><th>Arbeitsgang</th><th>Maschine</th>' +
          ...
          '<th class="z">gesamt</th><th>Programm</th><th></th></tr>';
  g.forEach((x, i) => {
    ...
    h += '<tr' + (schlecht ? ' class="warnzeile"' : '') + '>' +
      ...
      /* Das NC-Programm des Gangs (19.09.2026): O-Nummer der Monforts oder Name des Klartext-
         Programms - steht danach auf Laufkarte und Maschinenzettel. */
      '<td><input type="text" data-gp="' + i + '" value="' + wEsc(x.programm || '') + '" placeholder="O-Nr. / Name" style="width:110px"></td>' +
...
function wGaengeLesen(a){
  ...
  a.gaenge.forEach((g, i) => {
    ...
    const pg = t.querySelector('[data-gp="' + i + '"]');
    if(pg) g.programm = String(pg.value || '').trim();
  });
  gaengeAusgleichen(a);
...
function wAngebotAuftrag(){
  ...
  const a = auftragAusKalkulation(S.d, k, {
    ...
    programm:(S.cam && S.cam.programm && S.cam.programm.name) || '',   /* aus der CAM-Zeitdatei */
    heute:wHeute(),
...
function wLaufkarte(a){
  ...
  h += '<h2>Arbeitsg&auml;nge</h2><table class="papier">' +
    '<tr><th>Nr.</th><th>Arbeitsgang</th><th>Maschine</th><th>Programm</th><th class="z">R&uuml;sten</th>' +
  ...
  h += '<tr class="summe"><td colspan="4">zusammen</td>' +
...
function wZettel(mid, tage){
  ...
  z.tage.forEach(t => {
    ...
    h += '<table class="papier"><tr><th>Auftrag</th><th>Teil</th><th>Arbeitsgang</th><th>Programm</th>' +
    ...
    t.posten.forEach(p => {
      h += '<tr><td>' + wEsc(p.auftrag) + '</td><td>' + wEsc(p.teil) + '</td>' +
        ...
        '<td class="leer"></td></tr>';
      if(p.bemerkung)
        h += '<tr><td></td><td colspan="8" class="klein">' + wEsc(p.bemerkung) + '</td></tr>';
    });
    h += '</table>';
  });
  if(leer === z.tage.filter(t => t.kapazitaet).length)
    h += '<p class="klein">In diesem Zeitraum liegt nichts auf dieser Maschine.</p>';
  /* Der Weg zum Programm (19.09.2026, Luecke 9): das NC-Programm liegt in der Ablage des
     DNC-Kaestchens der Maschine - der Zettel sagt es, sobald ein Gang eines traegt. */
  if(z.tage.some(t => t.posten.some(p => p.programm)))
    h += '<p class="klein">NC-Programme: in der Ablage des DNC-K&auml;stchens dieser Maschine &mdash; dort <i>In die Maschine laden</i>. ' +
      'Fehlt eines, in der Dreh-App <i>NC sichern</i> und aufs K&auml;stchen legen.</p>';
